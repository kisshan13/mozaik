...
export abstract class Processor<TInput, TEvent extends BaseEvent> implements EventPublisher<TEvent> {
	...
	unsubscribe(eventType: string, listener: EventObserver<TEvent>): void {
		const observers = this.observers.get(eventType) || []
		this.observers.set(eventType, observers.filter(observer => observer !== listener))
	}
